...
public class ProductController : Controller {
    ...
    public IActionResult Index() {
        try {
            ...
            var productListViewModels = productModels.Select( product => new ProductListViewModel {
                                                             Product = product,
                                                             Gender = product.Gender,
                                                             Categories = product.ProductCategories
                                                                 .Select( pc => pc.Category )
                                                                 .ToList(),
                                                             Colors = product.ProductColors.Select( pc => pc.Color )
                                                                             .ToList(),
                                                             Sizes = product.ProductSizes.Select( ps => ps.Size )
                                                                            .ToList(),
    ...
    public IActionResult Details( int id ) {
        ...
        var stockData = product.StockProducts.ToDictionary(
        ...
        var viewModel = new ProductDetailViewModel {
            Product = product,
            Gender = product.Gender,
            Images = product.ProductImages.OrderBy( i => i.ImageOrder ).ToList(),
            Categories = product.ProductCategories.Select( pc => pc.Category ).ToList(),
            AvailableColors = product.ProductColors.Select( pc => pc.Color ).ToList(),
            AvailableSizes = product.ProductSizes.Select( ps => ps.Size ).ToList(),
    ...
    public IActionResult Edit( int id ) {
        ...
        var viewModel = new ProductEditViewModel {
            ...
            SelectedColors = product.ProductColors.Select( pc => pc.ColorId ).ToList(),
            SelectedSizes = product.ProductSizes.Select( ps => ps.SizeId ).ToList(),

            // Carrega as imagens que já existem
            ExistingImages = product.ProductImages.OrderBy( pi => pi.ImageOrder ).ToList(),
            Stock = product.StockProducts.Select( s => new StockEditViewModel {
                                   StockId = s.StockId,
    ...
    [ HttpGet ]
    public IActionResult GetSizesJson() {
    ...
    public async Task<IActionResult> Edit( ProductEditViewModel viewModel ) {
        ...
        if ( viewModel.SelectedColors == null )
            viewModel.SelectedColors = new List<int>();

        if ( viewModel.SelectedSizes == null )
            viewModel.SelectedSizes = new List<int>();

        if ( viewModel.Stock == null )
            viewModel.Stock = new List<StockEditViewModel>();

        if ( viewModel.ImagesToDelete == null )
            viewModel.ImagesToDelete = new List<int>();

        if ( viewModel.NewImages == null )
            viewModel.NewImages = new List<IFormFile>();
            ...
            try {
                ...
                if ( viewModel.NewImages.Any() ) {
                    ...
                    int lastOrder = productToUpdate.ProductImages.Any()
                        ? productToUpdate.ProductImages.Max( i => i.ImageOrder ?? 0 )
                        : 0;
        ...
        if ( ModelState.IsValid ) {
            try {
                ...
                if ( viewModel.PImages.Any() ) {
                    ...
                    // MELHORIA: Trocado 'foreach' por 'for' para salvar a ordem
                    for ( int i = 0; i < viewModel.PImages.Count; i++ ) {
                        var imageFile = viewModel.PImages[ i ];
